...
// Rend un court clip (WRONG YOURE) avec le STYLE COURANT et renvoie {frame,style}
async function renderStyleFrame(){
  const raw=latestRaw(); if(!raw)return null;
  const {renderLocal}=require('./render_local');
...
async function captureBaseline(){ const f=await renderStyleFrame(); editPrevFrame=f?f.frame:null; }
