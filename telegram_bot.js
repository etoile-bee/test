...
async function showSettings(){
  ...
  await send(`⚙️ <b>Réglages sous-titres</b>\n\n🔤 Police: <b>${fontLabel(font)}</b>\n📝 Taille: ${size}px\n📍 Position y: ${y}\n🔍 Zoom: ${zoom}x\n💬 Sous-titres: <b>${subs?'ON':'OFF (vidéo propre pour TikTok)'}</b>`,[
    ...
    [{text:'🔍+ Zoom',callback_data:'S_Z_UP'},{text:'🔍- Dézoom',callback_data:'S_Z_DN'}],
    [{text:subs?'💬 Sous-titres: OFF':'💬 Sous-titres: ON',callback_data:'S_SUBS'}],
    [{text:'👁 Aperçu (/preview)',callback_data:'S_PREVIEW'}],
  ]);
}
function patchWF(fn){
...
async function runPreview(){
  try{
    ...
    const st=r.style;
    const cap=`🔤 ${fontLabel(st.font)} • ${st.fontSize}px • y=${st.oy} • zoom ${st.baseZoom||1} • 💬 ${st.subs?'ON':'OFF'}`;
    await sendImg(f1,cap).catch(()=>{});
    await sendImg(f2,'👁 Aperçu (pendant zoom) — ajuste via /settings').catch(()=>{});
...
await send('Ready to generate video?',[
    ...
    if(d==='S_PREVIEW'){await runPreview();return;}
    if(d.startsWith('S_')){
...
tg('setMyCommands',{commands:[ /*cmdmenu v1 : les commandes apparaissent dans le menu "/" de Telegram*/
  ...
  {command:'settings',description:'⚙️ Police, taille, position, zoom, sous-titres ON/OFF'},
  {command:'preview',description:'👁 Aperçu gratuit du style courant'},
