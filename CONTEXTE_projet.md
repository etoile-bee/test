...
> 🚦 **08/06 03h — REPRISE AU PROPRE EN COURS : suivre `PLAN_REPRISE.md` (phases 0→4, on coche dans l'ordre, un seul chantier à la fois).** Audit complet dans `RAPPORT_AUDIT_2026-06-08.md`. Porte d'entrée Telegram unique : `/studio`.
...
- 🔒 RÈGLE MONO-SESSION (08/06, GRAVÉE — Phase 1 GEL) : **UNE SEULE session Claude modifie le code à la fois. Toute autre session ouverte est en LECTURE SEULE** (elle peut lire, expliquer, proposer — jamais éditer/commit/restart). Avant d'écrire, la session active s'assure qu'elle est la seule à écrire ; les autres attendent leur tour.
  - LEÇON DE LA DIVERGENCE D'HIER (07-08/06) : deux sessions (Dispatch + Cowork) ont édité le même repo en parallèle → le HEAD bougeait tout seul, `subtitle_style.js` a divergé (45px/OY0.25 documenté ≠ 76px/OY0.37 dans le fichier), et il a fallu une session entière de réconciliation pour vérifier que les garde-fous n'avaient pas été perdus. Coût évitable. D'où : mono-session + verrou git (`commit-msg` + `LOCKED_FILES.txt`) sur les fichiers de style.
...
- 📓 **Suivi dev (Phase 2)** : le journal `podcast-outputs/dev_journal.txt` (iCloud) reste le backup écrit en continu. Le **flux Telegram continu (dev-feed) est ARRÊTÉ** (`pm2 stop dev-feed`, choix Etoile : backup à la demande, plus de streaming). Pour pousser les N dernières lignes sur Telegram **à la demande** : `tools/send_journal.sh [N]` (défaut 15, avec notification). Pour réactiver le flux continu un jour : `pm2 start dev-feed && pm2 save`.
...
- 📋 AUDIT COMPLET 08/06 ~02h30 : voir RAPPORT_AUDIT_2026-06-08.md. Patchs P0+P1 appliqués dans la foulée (node --check OK, PAS RESTART) : confirmations 💰 sur NL_HD/RETRY/OTHER (via nlPayRecap), vrais coûts vidéo depuis lookbook.pricing dans estimateCost, erreurs API humanisées (apiNice) + sujet local de secours (TOPIC_IDEAS) quand l'API est down, NEW_GO ouvre LA CARTE (ancien chooser Sur-mesure/Aléatoire débranché — handlers MM_/T_/D_/L_ gardés mais plus atteignables), bloc 3 photo : + Refaire pareil/Autre look, genFolders restaurés au boot (re-scan outputs/generations, remap gfIdx par chemin, ADDPART bloqué proprement sur les restaurées), tests locaux → outputs/tests/ (plus /tmp), hooks/catégorie/technique/aide convertis cardMenu (anti-empilement). ⚠️ Limite connue : les boutons GF_* des VIEUX messages vidéo (avant restart) peuvent viser le mauvais dossier — utiliser le bloc RÉSULTATS. ⚠️ Commits git en attente (verrous .lock) : sur le Mac `rm -f ~/podcast-workflow/.git/*.lock && git add -A && git commit -m "P0+P1 audit"`.
...
- **2026-06-08 (session « suivi à distance » — PHASE 1 GEL & STABILISATION → tag `STABLE-1`)** : Etoile suit depuis son iPhone. (1) État des lieux : garde-fous TOUS présents sur HEAD (sanitizeTTS+ceinture, validTTS/TTS_BAD anti-status, genAbort, garde renderStyleFrame null, anti-dépense récap+ttsCheck) → rien à regreffer. (2) Arbitrage sous-titres : 3 images test locales gratuites (45/0.25 · 76/0.33 · 76/0.370) → **Etoile choisit 76px/OY 0.370** (`subtitle_style.js`, commit `1304db2`). Couleur V5 inchangée, ré-arbitrage couleur reporté. (3) Verrou physique : hook git `commit-msg` + `LOCKED_FILES.txt` (subtitle_style.js, color_style.js) → modif sur jeton `[UNLOCK-ETOILE]` seulement (testé). (4) Règle MONO-SESSION gravée. (5) Infra suivi : journal iCloud `podcast-outputs/dev_journal.txt` + **miroir Telegram live** (pm2 `dev-feed`, préfixe 🛠) + **keep-awake** (pm2 `caffeinate -dims`). Restart bot OK, smoke test OK (online, polling pending 0, /go câblé). **Tag `STABLE-1`.**
