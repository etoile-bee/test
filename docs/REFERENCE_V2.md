...
### Cycle conformité COMPLET (4 axes Etoile) — preuves
- **Axe 1 — INTÉGRITÉ inter-écrans** (`tools/test_v4r_integrity.js` **20/0**, 110 projets) : bijection n° (1..N, zéro doublon, zéro trou, stable par cree_le) · Récents liste chaque projet UNE fois, aucun ne disparaît · sur les **19 pages** vignette==bouton==projet réel · **bouton n°X ouvre le projet n°X** (1·2·55·109·110 sur plusieurs pages) · **médias/script/légendes rattachés au BON projet** (zéro fuite) · **MÊME n° projet sur les 5 écrans** (Fichiers/Prêt/Publiés/Publication/Résultat, header == `r0ProjNum` via ctx réel) · Prêt/Publiés : boutons positionnels == médias du projet courant.
- **Axe 2 — GRILLES/NUMÉROTATION toute volumétrie** : planches rendues & **regardées** sur pages **1·2·5·10·19 + dernière** (`assets_r/_AUDIT_page2/10/19.jpg`, `_AUDIT_recents_DERNIERE_page.jpg`) — chaque tuile distincte (cover propre OU placeholder), n° brûlé == bouton, badge VIDEO cohérent, page partielle paddée.
- **Axe 3 — ERGONOMIE par écran** (`tools/audit_ergonomie_nuit.js` 80/0, seuil **≤18 car.**) : 20 écrans audités — libellés ≤18, rangées ≤2 (grilles 3/ligne tolérées), Retour/Accueil présents, aucun écran surchargé, aucune coupure absurde. **2 anomalies trouvées+corrigées** : « 🚫 Aucune (image de base) » (24c) → « 🚫 Aucune » ; « ✨ Générer maintenant » (20c) → « ✨ Générer » (l'écran indique déjà « Dépense réelle au clic »). Tableau écran-par-écran : voir sortie de l'outil (tous ✅).
- **Axe 4 — DOC redéroulé** : parcours complet reconfronté ; points conformes laissés, AN3 + ergo réouverts puis corrigés, aucune nouvelle anomalie. Cockpit conforme, zéro régression inter-écrans.
...
Sweep **42/0** · audit_visual **9/0** · audit_flow **27/0** · audit_final **13/0** · audit_ergonomie **80/0** · audit_integrity **14/0** · audit_parcours **0 écart** · états vides **9/0** · verrous intacts. Anomalies nuit trouvées+corrigées+recontrôlées : **AN1** (planches Prêt/Publiés), **AN2** (herméticité faux-vert), **AN3** (grille photo empruntée → placeholder distinct), **ergo** (libellé Aucune trop long). Lot demandes Etoile **S·T·U·V·W2·X·C2** ✅. Cycle conformité COMPLET (4 axes : intégrité inter-écrans, grilles toute volumétrie regardées, ergonomie par écran, doc redéroulé) : **aucun écart restant**. Prod LIVE = `fca1d22` ; build conforme courant à déployer sur GO.
...
| AA | **Brouillons VIDES qui noient tout** — Récents/Archives = 112 projets dont ~111 brouillons vides → grilles pleines de placeholders, vrai contenu enterré (IMG_3719-3722) | (1) les vues de navigation listaient TOUS les projets, vides compris ; (2) la navigation/affichage CRÉAIT un brouillon vide à chaque fois (`r0Render`/`r0Dispatch` en `r0Cur(true)`) → accumulation | (1) **AA.1** `r0HasRealMedia` : Récents n'affiche QUE les projets avec ≥1 photo/vidéo réelle ; vides masqués + comptés (caption « N brouillon(s) vide(s) masqué(s) »). (2) **AA.2** `r0Render` ne crée plus ; `r0Dispatch` ne crée que hors navigation/lecture (liste `_navOnly`) → un projet naît seulement sur vraie action (sélection/génération), et un brouillon vide existant est RECYCLÉ plutôt que dupliqué. (3) **AA.3** `tools/cleanup_empty_drafts.js` (dry-run par défaut, `--apply` réversible vers `_corbeille_vides/`, `--undo`) — **non lancé**. | test_v4r_emptydrafts 9/0 (38 projets : 8 réels affichés, 30 vides masqués ; nav→0 création ; sélection recycle un vide) + planche `assets_r/_AUDIT_recents_REEL_seulement.jpg` (que du réel) |
| AN3 | **Grille : photo EMPRUNTÉE** — 6 projets DIFFÉRENTS affichaient la MÊME image (terrain IMG_3717, page 2) | `r0CoverFile` retombait, pour un projet sans image, sur `r0RealImages` = la photo la PLUS RÉCENTE du patrimoine GLOBAL ; or beaucoup de projets sont des brouillons vides → tous la même photo | grille Récents/Archives : `r0OwnCover(p)` = cover STRICTEMENT propre (image propre, sinon poster de sa propre vidéo, sinon **null**) ; si null → `r0PlaceholderTile(n°, libellé)` = vignette ardoise DISTINCTE (n° brûlé + « sans image » + libellé). **Jamais** la photo d'un autre projet. | test_v4r_gridcover 5/0 + planche rendue `assets_r/_AUDIT_grid_cover_mix.jpg` (vide→placeholder, plein→cover propre, 6 tuiles distinctes) |
