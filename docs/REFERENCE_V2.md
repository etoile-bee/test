...
- **Axe 3 — ERGONOMIE par écran** (`tools/audit_ergonomie_nuit.js` 80/0) : 20 écrans audités — libellés ≤22 car., rangées ≤2 (grilles 3/ligne tolérées), Retour/Accueil présents, aucun écran surchargé, aucune coupure absurde. **1 anomalie trouvée+corrigée** : libellé « 🚫 Aucune (image de base) » (24c) → « 🚫 Aucune » (le « Actuel » indique déjà « image de base »).
