...
- **E92** — **[GATE BLOQUANT — PRIORITÉ CRITIQUE n°1] QC identité/anatomie sur l'IMAGE SOURCE, AVANT toute génération vidéo payante.** `[Etoile 09/06 ; renforcé C4 10/06]`
  - **🔧 RENFORCEMENT C4 (Etoile 10/06)** : contrôle **OBLIGATOIRE avant TOUTE génération payante** (pas seulement la vidéo), vérifiant **identité du personnage · cohérence générale · conformité à la référence · conformité au look attendu**. Actions : **🔄 Régénérer / 🎨 Éditer / ✅ Valider malgré l'alerte**. La vérification (verdict + qui + quand) est **ENREGISTRÉE dans le dossier projet** et **apparaît dans le rapport qualité** ([[E93]].2 rapport QC). Placement : dans **FINALISER**, juste avant Final HD.
...
- **E108** — **[RÈGLE DIRECTRICE] NAVIGATION UNIVERSELLE.** Tous les écrans du workflow utilisent les **mêmes commandes au comportement strictement identique** : **⬅ Retour** (= parent direct) · **✅ Valider** (= action principale qui FAIT AVANCER : valide l'étape courante et débloque/affiche l'étape suivante) · **🏠 Accueil** (= racine) · **⏹ Stop** (= arrête l'opération en cours) · **🔄 Restart** (= redémarrage propre) · **❓ Aide** (= aide **contextuelle** de l'écran courant). Si un choix est **obligatoire**, **✅ Valider reste désactivé** tant que le choix n'est pas fait. Handlers **centralisés** (un seul comportement partout). `[Etoile 09/06 ; C1 10/06 : « ➡ Suivant » remplacé par « ✅ Valider », même rôle d'avance + désactivé-tant-que-pas-choisi]`
...
- **E115** — **[RÈGLE DIRECTRICE] AVANCE PAR VALIDER + CONSERVATION DES ÉTAPES SUIVANTES.** Boutons **⬅ Retour · ✅ Valider · 🏠 Accueil partout** ; **état sauvé à chaque étape** ([[E114]]). Revenir sur une étape antérieure, la modifier, puis **✅ Valider** **reprend la suite DÉJÀ EXISTANTE** — **sans effacer ni régénérer l'aval** (ex. PHOTO→Look→Image→Script→Montage : revenir sur Look ne fait **pas** disparaître Script/Montage). **Sauf** si la modif rend l'aval **vraiment incompatible** → le système **DEMANDE** : « Cette modification peut impacter les étapes suivantes. **Conserver / Mettre à jour / Régénérer ?** ». **Aucune perte d'étape suivante sans validation explicite.** Étend [[E114]]. `[Etoile 09/06 ; C1 10/06 : « ➡ Suivant » → « ✅ Valider », conservation de l'aval inchangée]`
  - **CRITÈRE D'ACCEPTATION L0-2** : Retour→modif→Valider conserve l'aval ; incompatibilité → invite Conserver/Mettre à jour/Régénérer (jamais d'effacement silencieux).
...
- **E120** — **[GOUVERNANCE/QUALITÉ — NON NÉGOCIABLE] CONFORMITÉ AUX EXIGENCES VALIDÉES.** Discipline **obligatoire** avant **chaque** implémentation, checkpoint, auto-audit [[E118]] et livraison : relire **SYSTÉMATIQUEMENT** la liste **complète** des exigences/décisions validées/invariants/demandes historiques (E1→E120 + invariants : principe directeur, dossier projet, média actif, bloc unique, Historique/Prêt-à-poster, symétrie, Valider→suivant, image brute, fichiers verrouillés/fidélité, etc.). **Une exigence validée reste valide jusqu'à décision explicite d'Etoile de la modifier/supprimer** — elle ne doit **JAMAIS** disparaître par oubli, être déplacée, réinterprétée, ou cassée par une évolution. **Aucune livraison n'est prête tant que 100 % des exigences validées ne sont pas vérifiées explicitement.** Tout **CONFLIT** entre une exigence existante et une nouvelle évolution doit être **REMONTÉ AVANT implémentation**, jamais découvert après livraison. **`EXIGENCES.md` = document de référence VIVANT, consulté avant chaque dev/audit/livraison.** Gouverne [[E116]] [[E117]] [[E118]] [[E119]]. `[Etoile 10/06]`
  - **CRITÈRE** : l'auto-audit [[E118]] inclut obligatoirement un **tableau de CONFORMITÉ exhaustif** { exigence (E1→E121 + invariants) · statut conforme/non conforme · justification · preuve de test · régression oui/non } ; **aucune livraison déclarée prête sans ce tableau à 100 % vérifié** ; tout conflit détecté est remonté à Etoile **avant** d'implémenter.
- **E121** — **[RÈGLE DIRECTRICE — NON NÉGOCIABLE] SOURCE DE DONNÉES UNIQUE (RÉCENTS = VUE de l'HISTORIQUE).** Il y a **UN SEUL magasin de projets** (l'Historique = mémoire complète et permanente, [[E52]]/[[E93]]/[[E94]]). **RÉCENTS n'est PAS une entité de données** : c'est une **VUE FILTRÉE** (projets récents / en cours) **sur cette même source**. De même, Prêt-à-poster et les vues d'états ([[E110]] : Brouillons/En cours/Terminés/Archivés) sont des **filtres/vues** du même magasin, **jamais des stockages séparés**. **INTERDICTION** de créer deux systèmes de données qui se désynchronisent (cause historique des frontières legacy). Toute liste (RÉCENTS, Historique, Prêt-à-poster, états) lit le **même** magasin de dossiers projet. Gouverne [[E104]] [[E105]] [[E110]] ; arbitrage C2. `[Etoile 10/06]`
  - **CRITÈRE** : une seule fonction de lecture du magasin projets ; RÉCENTS/Historique/Prêt-à-poster/états = filtres sur son résultat ; zéro duplication de stockage ; un changement sur un projet est **immédiatement** reflété dans toutes les vues.
- **E122** — **[RÈGLE DIRECTRICE — GARDE-FOU CÂBLAGE] LES PRINCIPES MÉTIER PRIMENT SUR L'INTERFACE ; l'UI NE DÉTIENT JAMAIS L'ÉTAT.** Pendant **tout** le câblage Telegram (zone à risque de régressions) : **1)** le **DOSSIER PROJET reste la SOURCE DE VÉRITÉ** ; le **média actif** est piloté par le dossier (manifest), pas par l'écran. **2)** Les **vues Telegram ne deviennent JAMAIS la source de vérité**. **3)** Bibliothèques = **vues** ; **Récents / Historique / Prêt-à-poster = vues** ([[E121]], source unique). **4)** **AUCUN état critique ne vit uniquement dans l'interface** : tout état important réside dans le **manifest/dossier**, jamais seulement dans un message/handler/variable d'UI ; l'UI ne fait que **LIRE/ÉCRIRE** le dossier. **But** : ne pas recréer, en raccordant l'UI, les problèmes legacy qu'on élimine. **5)** **Tout ARBITRAGE FONCTIONNEL nécessaire pendant le câblage est REMONTÉ IMMÉDIATEMENT à Etoile** — **aucune décision produit cachée dans l'implémentation**. Gouverne [[E107]] (strangler-fig), [[E121]] (source unique), et le câblage cockpit-v4. `[Etoile 10/06]`
  - **CRITÈRE** : à chaque sous-étape de câblage, vérifier que l'UI **ne fait que lire/écrire** le dossier projet (jamais détenir un état critique) ; le seul état d'UI admis = des **pointeurs** (projet actif, étape courante) **reconstructibles** depuis le manifest ; tout besoin d'arbitrage produit = remonté, pas codé en douce.
- **E123** — **[RÈGLE DIRECTRICE] LIVRABLES EXPLICITES (la sélection Image/Vidéo porte sur le LIVRABLE, pas sur le parcours).** Conforme [[E121]]/[[E122]] (modèle A) : **un seul projet**, aucun nouvel embranchement — la différence Image/Vidéo est le **LIVRABLE**, pas le flux. `[Etoile 10/06]`
  - **FINALISER = SÉLECTION DES LIVRABLES** : « Livrables : ☑ Image ☑ Vidéo » (cochés par défaut). Décocher Vidéo ⇒ projet **image seule**. Garder les deux ⇒ l'image validée devient **automatiquement** la source de la vidéo. Le projet = dossier pouvant produire **image seule / vidéo seule / les deux**. **Jamais** « projet image » vs « projet vidéo ».
  - **VALIDATION D'IMAGE EXPLICITE (étape réelle, pas auto)** : après génération → aperçu → l'utilisateur décide le devenir de CHAQUE image : **garder** (→ image active du projet) · **régénérer** · **éditer** · **rejeter** (ne part nulle part) · **variante/candidate** (trace, **pas** livrable) · **promouvoir en LIVRABLE image et/ou SOURCE vidéo** (action explicite).
  - **RÈGLE CRITIQUE** : une image **NON validée** (ratée / candidate / rejetée) **NE DOIT JAMAIS** entrer automatiquement dans **Prêt-à-poster** ni dans les **LIVRABLES**. Les LIVRABLES ne contiennent **QUE** ce qui est explicitement validé. L'Historique peut garder la **trace technique** (candidates/variantes/rejetées) — ce n'est **pas** un livrable.
  - **MANIFEST** : distinguer (a) **candidates/variantes/rejetées** (trace technique) vs (b) **LIVRABLES** (validés explicitement : image et/ou vidéo). `media_actif` = l'image **gardée** ; `livrables` = sous-ensemble **validé** ; rien n'entre en livrable/Prêt-à-poster sans validation explicite.
  - Gouverne [[E37]] (sélection image) [[E57]]/[[E96]] (prêt-à-poster/validation) ; étend [[E92]] (QC avant dépense).
  - **CRITÈRE** : aucune image non validée dans livrables/Prêt-à-poster (test) ; FINALISER coche Image+Vidéo par défaut ; décocher Vidéo ⇒ image seule ; garder les deux ⇒ image = source vidéo auto ; manifest sépare candidates/variantes/rejetées vs livrables.
- **E124** — **[RÈGLE DIRECTRICE — CRITIQUE] ÉTANCHÉITÉ INTER-PROJETS (chaque projet est HERMÉTIQUE).** Tous les réglages d'un projet — réglages image, filtres, **colorimétrie**, crop, améliorations, options de rendu, QC, variantes, livrables — vivent **dans son dossier projet** et **NE FUITENT JAMAIS** vers un autre projet : **jamais globaux, jamais hérités silencieusement, jamais propagés** d'un projet à l'autre (A4). C'est la correction structurelle de l'incident « filtre couleur ». Au rendu, les réglages du projet sont appliqués **transitoirement** (les fichiers verrouillés `render_local`/`subtitle_style`/`color_style` restent inchangés). `[Etoile 10/06]`
  - **CRITÈRE (test permanent E118)** : un **TEST D'ÉTANCHÉITÉ INTER-PROJETS** obligatoire démontre qu'aucun réglage/filtre/colorimétrie/crop/amélioration/option de rendu ne peut fuir d'un projet vers un autre (projet B créé après réglages sur projet A → B a les défauts neutres). Ce test est **critique et permanent**.
- **E125** — **[RÈGLE DIRECTRICE] DOSSIER LIVRABLE AUTONOME.** Une **version validée** devient un **vrai DOSSIER LIVRABLE autonome**, immédiatement récupérable/publiable. Hiérarchie : **Projet → Versions → Version validée → Dossier livrable**. Chaque dossier livrable contient **TOUT** le nécessaire à la publication : **vidéo finale · image(s) source · script · LÉGENDE COURTE · LÉGENDE LONGUE · HASHTAGS · paramètres · infos projet · exports · RAW (si dispo)**. Objectif : dès qu'un contenu arrive en **Prêt-à-poster**, tout est là **sans reconstruire le contexte** (publication TikTok rapide facilitée, auto-post hors lot A5). Le **projet reste le CONTENEUR UNIQUE** : images/variantes/vidéos/montages/scripts/légendes/exports **tous regroupés** dans le même dossier projet, jamais éparpillés. Étend [[E93]] [[E57]] [[E123]]. `[Etoile 10/06]`
  - **3 ÉTATS (renforce [[E123]])** : (1) **Candidate/Variante** — (2) **Média actif** — (3) **Livrable validé**. Trois états **DIFFÉRENTS** ; **aucune génération ne devient automatiquement un livrable** ; un livrable n'existe **QU'APRÈS validation explicite**.
  - **CRITÈRE** : à l'entrée en Prêt-à-poster, un **dossier livrable complet** existe (vidéo/images/script/légende courte+longue/hashtags/params/infos/exports/RAW) — **test de complétude**.
- **E126** — **[GOUVERNANCE — extension [[E120]]] ARBITRAGE PROACTIF PAR MODULE.** Pour **CHAQUE module important**, **AVANT implémentation**, remonter : les **décisions non prises** · les **options possibles** · la **recommandation** · les **impacts UX**. Etoile ne doit pas être la seule à faire émerger les zones grises. Tenir un **registre « zones grises / arbitrages par module »** (`docs/ZONES_GRISES.md`) et le remonter **en amont**. `[Etoile 10/06]`
  - **CRITÈRE** : aucun module important implémenté sans que ses zones grises aient été listées (options + reco + impact UX) et remontées AVANT ; registre tenu à jour.

## U. MODÈLE MÉTIER VALIDÉ — LES 6 PILIERS (référence absolue, ajout 2026-06-10)

- **E127** — **[GOUVERNANCE — RÈGLE DE DÉRIVATION PAR LES PILIERS — NON NÉGOCIABLE] LE MODÈLE MÉTIER = 6 PILIERS EST LA RÉFÉRENCE ABSOLUE DONT TOUTE LA FORME DÉRIVE.** Etoile a **validé le modèle métier** (10/06) après une longue phase de découverte : **① INTENTION** (boussole : message · émotion · personnage · public · objectif/destination de publication — plateforme/format/durée — présente en permanence, courte, modifiable) · **② COHÉRENCE** (l'outil NE JUGE PAS le goût ; il rend la cohérence **lisible** et **signale la dérive sans décider**) · **③ MÉMOIRE** (le projet conserve le **POURQUOI** : intention, variantes testées, choix faits **+ raison**, version retenue — relisible des mois après) · **④ CAPITALISATION** (promouvoir/réutiliser des briques : look, décor, preset, style sous-titres, enchaînement) · **⑤ CONFIANCE** (prévisibilité par **contrat** : effet explicite **avant** action ; valider = exactement l'annoncé ; le verrou tient ; la reprise restaure l'état **EXACT** ; **rien en silence**) · **⑥ CONTRÔLE** (tout **réversible** jusqu'à la publication : revenir, comparer, restaurer, **réactiver une variante écartée**, **garder/mettre à jour/régénérer AU CHOIX** — jamais auto ; **publier = seul acte irréversible**). **PHRASE CENTRALE** : « Je pilote la transformation d'une **intention** en contenu **publiable**, par une suite de décisions **cohérentes, réversibles et mémorisées**, dans un système en qui j'ai **confiance** — jusqu'au seul acte définitif : **publier**. » `[Etoile 10/06]`
  - **RÈGLE DE JUSTIFICATION** : **chaque** écran/bouton/automatisation/élément du cockpit doit être **JUSTIFIÉ par ≥ 1 des 6 piliers**. S'il ne sert ni l'intention, ni la cohérence, ni la mémoire, ni la capitalisation, ni la confiance, ni le contrôle → **il n'a pas sa place, on le retire**.
  - **INTENTION = OBJET DE PREMIER NIVEAU** : message · émotion · personnage · public · objectif de publication vivent dans le **manifest** au même rang que référence/look/décor/prompt, **présents en permanence** à l'écran. (Constat fondateur : l'intention **MANQUE** aujourd'hui dans le moteur — déficit pilier 1 à corriger en priorité.)
  - **QUESTION CENTRALE RÉCURRENTE** : l'outil ramène **toujours** à « **ce que je produis sert-il encore l'intention de départ ?** ». Si non, il aide à **IDENTIFIER la pièce qui dérive** parmi : **message · image · look · décor · voix · script · montage · rythme · légende · publication** — il rend la dérive **VISIBLE** puis **LAISSE CHOISIR**. **JAMAIS de décision à sa place.**
  - **CRITÈRE** : matrice de justification tenue (élément → pilier(s)) ; toute fonction non justifiée retirée ; l'intention est un objet de 1er niveau présent en permanence ; un mécanisme de cohérence/dérive **lisible et non décisionnel** existe. Réf. forme dérivée : `docs/COCKPIT_PAR_PILIERS.md`. Gouverne **tous** les E précédents (la forme dérive du modèle).

- **E128** — **[GOUVERNANCE — ÉTANCHÉITÉ DES RESPONSABILITÉS] 3 RÈGLES D'ÉTANCHÉITÉ GRAVÉES (architecture cap/pilotage à 5 espaces).** Issues de l'audit de séparation (verrouillage 2026-06-11), à respecter dans **toute** réalisation future : **(1)** la règle **M15 « structurant »** (change/remplace une direction · fixe un devenir · valide un état · modifie le sens) est une **règle CANONIQUE UNIQUE**, **référencée partout** (Conscience pour classer une *tension* ; Pilotage pour qualifier une *action*), **jamais redéfinie localement** ; **(2)** le **« prochain geste »** proposé par le **Pilotage** est un **strict mapping** de la **focalisation calculée par la Conscience** — **le Pilotage ne recalcule JAMAIS** le carrefour ni aucune lecture (situation/titre/tensions/miroir) ; **(3)** l'**Atelier** ne **dépose que des faits-médias** (média produit à l'état `candidate` + RAW) et **JAMAIS un statut de devenir** (retenu/actif/validé/publié) — ce **verdict** appartient **exclusivement au Pilotage exécutant l'auteur**. Gouverne l'architecture (`docs/ARCHITECTURE_SYSTEME.md`), la cartographie (`docs/CARTOGRAPHIE_RESPONSABILITES.md`) et l'audit d'étanchéité (`docs/AUDIT_DE_SEPARATION_DES_RESPONSABILITES.md`). Étend [[E122]] (l'UI/les espaces ne détiennent jamais l'état) et [[E127]] (modèle/forme). `[Etoile 11/06]`
  - **CRITÈRE** : aucune réalisation ne duplique un calcul entre deux espaces ; M15 a **une seule** définition de référence ; le « prochain geste » dérive de la focalisation (jamais recalculé) ; l'Atelier n'inscrit **aucun** statut de devenir. Vert d'étanchéité (4 vérifs globales + 3 points explicites) **re-vérifié** avant livraison.

- **E129** — **[CLARIFICATIONS VERROUILLÉES DES LOIS I/II — conséquences acquises, PAS une règle nouvelle] RESPONSABILITÉ DU SENS.** Conséquences gravées de la frontière du miroir (= **A**, factuel/structurel) et de la matrice de responsabilité du sens, **non renégociables sauf contradiction démontrable** : **(1)** le système ne produit **JAMAIS** de tension émotionnelle ou de message par **interprétation** ; **(2)** ces tensions n'existent que comme **reflet d'un jugement enregistré** par l'auteur **OU** **proxy factuel + invitation à juger** ; **(3)** une **fracture du cap** n'existe qu'une fois **NOMMÉE** par l'auteur ; **(4)** une **polarité constitutive** n'existe qu'une fois **REVENDIQUÉE** par l'auteur (marqueur niv.3) ; **(5)** le **cap agi** peut être **OBSERVÉ** et **COMPARÉ** (divergence comptée) au cap déclaré, **jamais INTERPRÉTÉ** ; **(6)** la **focalisation** reste un **TRI DÉTERMINISTE** (stratification + 4 critères) ; **(7)** le **prochain geste** reste un **MAPPING STRICT** de la focalisation. **Le sens appartient à l'auteur (Loi II), la décision appartient à l'auteur (Loi I).** Réf. : `docs/FRONTIERE_DU_MIROIR.md`, `docs/MATRICE_RESPONSABILITE_DU_SENS.md`, `docs/STRUCTURE_CONSCIENCE.md`. Précise/verrouille [[E127]] (Conscience) et [[E128]] (étanchéité). `[Etoile 11/06]`
  - **CRITÈRE** : toute lecture « sémantique » produite par le système est soit un **proxy factuel + invitation**, soit le **reflet d'un enregistrement** de l'auteur ; **aucune conclusion de sens** ; tout manquement = violation de la **Loi II** (rejet d'office). **Socle / Conscience / étanchéité / responsabilité du sens = VERROUILLÉS.**

- **E130** — **[RÈGLE DE LECTURE — clarification verrouillée, PAS une règle nouvelle] TROIS VÉRITÉS QUI NE SE MÉLANGENT JAMAIS.** Conséquence acquise de l'architecture cap/pilotage : **l'UNIVERS = vérité du MONDE** (capital, règles, personnages, séries) · **le SOCLE = vérité du PROJET** (cap, mémoire, configuration, matière, versions, livrables) · **la CONSCIENCE = lecture DÉRIVÉE du projet** (situation/titre/signaux/focalisation/tensions/miroir, recalculables, jamais stockées). **Ces trois responsabilités ne se mélangent JAMAIS** : l'Univers n'écrit pas dans un projet, le Socle ne calcule pas de lecture, la Conscience ne détient aucune vérité. Réf. : `docs/ARCHITECTURE_SYSTEME.md`, `docs/STRUCTURE_SOCLE.md`, `docs/STRUCTURE_CONSCIENCE.md`, `docs/STRUCTURE_UNIVERS.md`. Précise/verrouille [[E122]] [[E128]] [[E129]]. `[Etoile 11/06]`
  - **CRITÈRE** : à toute étape, chaque information appartient à **exactement une** des trois vérités (monde / projet / lecture dérivée) ; aucun mélange ; **Socle / Conscience / étanchéité / responsabilité du sens / Univers = VERROUILLÉS** (non renégociables sauf contradiction démontrable).

- **🔒 CLÔTURE DE LA FONDATION (Etoile 11/06)** — sont **VERROUILLÉS et non rouvrables** (sauf **contradiction démontrable**) : **Socle · Univers · Conscience · Atelier · Pilotage · Cartographie · Étanchéité · Audit global.** **Fondation close** ; contre-vérification **VERTE** (AUD-G1 résolu : impact aval = dérivation Conscience consommée par le Pilotage). Réf. : `docs/STRUCTURE_SOCLE.md`, `STRUCTURE_UNIVERS.md`, `STRUCTURE_CONSCIENCE.md`, `STRUCTURE_ATELIER.md`, `STRUCTURE_PILOTAGE.md`, `CARTOGRAPHIE_RESPONSABILITES.md`, `AUDIT_DE_SEPARATION_DES_RESPONSABILITES.md`, `AUDIT_GLOBAL_5_ESPACES.md`, `CONTRE_VERIFICATION_AUD-G1.md`. *(Point ouvert hors fondation : calibration des signaux A/B, non gravée.)*

- **E131** — **[GOUVERNANCE — MÉTA-RÈGLE DE PROCESSUS] CADRE DE GOUVERNANCE DES ÉVOLUTIONS.** **⚠️ SIGNALEMENT HONNÊTE : E131 est lui-même une NOUVELLE RÈGLE (catégorie D dans ses propres termes), explicitement arbitrée et autorisée par l'auteur (Etoile, 11/06)** — c'est précisément l'arbitrage explicite que la règle impose. Il **ne touche aucun objet/invariant/comportement** du modèle métier : c'est une **méta-règle de processus**. Toute évolution proposée à partir de maintenant **DOIT être classée** : **A** Clarification d'un acquis · **B** Formalisation d'un acquis · **C** Correction d'une contradiction démontrée · **D** Nouvelle règle. **A/B/C = travail normal autorisé ; D = ARRÊT OBLIGATOIRE + arbitrage explicite de l'auteur AVANT intégration.** **En-tête obligatoire de TOUT nouveau document** : *Statut A/B/C/D · Acquis-source(s) · Modifie un élément déjà verrouillé : OUI/NON* (si OUI → C avec contradiction démontrée, ou D avec arbitrage). **Objectif : empêcher toute dérive conceptuelle progressive après la clôture de la fondation.** Réf. : `docs/GOUVERNANCE_EVOLUTIONS.md`. Étend [[E119]] [[E120]] [[E126]]. `[Etoile 11/06]`
  - **CRITÈRE** : aucun nouveau document/évolution sans **en-tête A/B/C/D + acquis-source + modifie-verrouillé OUI/NON** ; toute catégorie **D** est **arrêtée et arbitrée** avant intégration ; toute modification d'un élément verrouillé relève de **C** (contradiction démontrée) ou **D** (arbitrage).
...
> 🔒 **QUALITÉ/LIVRAISON FIGÉES — VALIDÉES par Etoile (2026-06-10)** : [[E118]] (protocole qualité avant chaque livraison — 12 étapes, 8 tests) · [[E119]] (méthode de livraison unique cohérente — design figé → implémentation complète → audit → auto-tests → livraison unique → tests utilisateurs) · [[E120]] (conformité aux exigences validées — relecture systématique E1→E121, tableau de conformité 100 %, conflits remontés AVANT impl) · [[E121]] (source de données unique — RÉCENTS/Prêt-à-poster/états = vues filtrées de l'Historique, jamais de stockage séparé) · [[E122]] (garde-fou câblage — l'UI ne détient jamais l'état ; principes métier > interface ; arbitrages remontés, jamais codés en douce) · [[E123]] (livrables explicites — sélection Image/Vidéo = LIVRABLE d'un projet unique ; validation d'image explicite ; aucune image non validée en livrable/Prêt-à-poster). · [[E124]] (étanchéité inter-projets — réglages/filtres/colorimétrie ne fuient jamais d'un projet à l'autre ; test d'étanchéité critique permanent).

---

_Total : 131 exigences (E1–E131 ; E131 = méta-règle de gouvernance des évolutions, catégorie D explicitement autorisée). Sections O (E74–E92) + P (E93–E96, E110) + Q (E97–E103) + S (E104–E109, E111–E117) + T (E118–E126) + U (E127 : MODÈLE MÉTIER — 6 PILIERS ; E128 : ÉTANCHÉITÉ DES RESPONSABILITÉS — 3 règles de l'architecture cap/pilotage ; E129 : RESPONSABILITÉ DU SENS — clarifications des Lois I/II ; E130 : RÈGLE DE LECTURE — 3 vérités qui ne se mélangent jamais [monde/projet/lecture dérivée]) = QUALITÉ, PROJET, TRAÇABILITÉ, FIDÉLITÉ, UX, ARCHITECTURE, DETTE, NAVIGATION, BROUILLONS, HISTORIQUE, UNICITÉ, PERSISTANCE, CONTINUITÉ, COHÉRENCE PARCOURS, GOUVERNANCE, LIVRAISON, CONFORMITÉ, MODÈLE MÉTIER, ÉTANCHÉITÉ, RESPONSABILITÉ DU SENS, RÈGLE DE LECTURE — priorité critique/élevée. Règles UX/gouvernance E108–E117 + qualité/livraison/conformité E118–E126 + modèle métier E127 + étanchéité E128 + responsabilité du sens E129 + règle de lecture E130 FIGÉS (validés Etoile)._
...
**Conformité aux exigences validées (OBLIGATOIRE avant chaque livraison)** `(E120)`
- ☐ **Tableau de CONFORMITÉ exhaustif** produit : pour CHAQUE exigence E1→E120 + invariants validés → { **statut** conforme/non conforme · **justification** · **preuve de test** · **régression** oui/non }.
- ☐ **100 %** des exigences validées vérifiées explicitement (aucune perdue/déplacée/réinterprétée/cassée).
- ☐ Tout **conflit** exigence↔évolution **remonté à Etoile AVANT implémentation** (jamais découvert après livraison).
- ☐ `EXIGENCES.md` relu **avant** ce dev/audit/livraison (document de référence vivant).
