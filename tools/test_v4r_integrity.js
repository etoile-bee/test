...
(async()=>{
  ...
  // ── F) Écrans PROJET (Fichiers/Publication/Résultat) portent le n° du projet ouvert ; Prêt/Publiés sont GLOBAUX (n° projet PAR vignette). ──
  const tag='n°'+wantNum;
  const capOf=()=>String((bot.markup()||{}).caption||'');
  await bot.tap('R0_RES');       const cF=capOf();   chk('F : Fichiers — header « '+tag+' » (projet ouvert)', cF.indexOf(tag)>=0);
  ...
  // ── G) Prêt/Publiés GLOBAUX (Option A) : numéro = POSITION, tap = OUVRE le projet parent (médias garde/publie toujours rattachés à un projet). ──
  await bot.tap('R0_HOME'); await bot.tap('R0_PRET');
  const cP=capOf();
  chk('G : Prêt à poster est GLOBAL (« tous projets »)', /tous projets/i.test(cP));
  const pretBtns=bot.buttons().filter(b=>/^R0_OPENMED_/.test(b));
  const pretLabels=bot.labels().filter(t=>/^📤 \d+$/.test(t));
  chk('G : Prêt — numéro = POSITION (« 📤 1,2,3… ») + tap ouvre projet (R0_OPENMED)', pretBtns.length>=1 && pretLabels.length===pretBtns.length);
  // tap une vignette -> ouvre un VRAI projet (le média est rattaché à son projet parent)
  await bot.tap(pretBtns[0]); chk('G : taper une vignette Prêt OUVRE son projet parent (curId = projet réel)', !!bot.curId() && bot.projNum(bot.curId())>=1);
