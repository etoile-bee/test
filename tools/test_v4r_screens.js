...
chk('Photo/Prompt [S/LOT2] : Prompt/Tenue/Décor + Faire une vidéo ; SANS Influences ; RÉFÉRENCE/Montage/Avatar/refs/format hors prépa', ['R0_PHB_prompt', 'R0_PHB_look', 'R0_PHB_decor', 'R0_PH_TOVIDEO'].every(c => has(php, c)) && !has(php, 'R0_PHB_influences') && !has(php, 'R0_PHB_reference') && !has(php, 'R0_PH_MONTAGE') && !has(php, 'R0_PHB_avatar') && !has(php, 'R0_PHB_refs') && !has(php, 'R0_PHB_params'));
...
chk('Vidéo/Résultat = HUB : Lég.courte/longue(copie)/Partie/Refaire/Modifier/Régénérer/Publier/Fichiers/Garder/Accueil', ['R0_FULLTEXT_legc', 'R0_FULLTEXT_legl', 'R0_VI_PART', 'R0_VI_CREATE', 'R0_VI_EDIT', 'R0_VI_REGEN', 'R0_PUB', 'R0_RES', 'R0_VI_KEEP', 'R0_HOME'].every(c => has(vir, c)));
...
chk('point5 : sélecteur Look = libellés propres (pas de JSON brut)', (() => { const sp = NAV.blockSpec({ screen: 'photo', key: 'look' }, fimg, ctx); const opts = [].concat.apply([], sp.optionRows || []).concat(sp.options || []); return opts.every(o => !/[{}\[\]]/.test(o.text)); })());
...
chk('D [P4bis] : bloc sous-titres = 3 réglages EXACTS (Police · Taille · Hauteur), legacy', (() => { const v = NAV.view({ screen: 'block', block: { screen: 'video', key: 'soustitres' } }, fvid, {}); const c = cbs(v); const has = ['R0_SET_stfont_archivo', 'R0_SET_stfont_classique', 'R0_SET_stsize_S', 'R0_SET_stsize_M', 'R0_SET_stsize_L', 'R0_STOY_up', 'R0_STOY_dn'].every(x => c.indexOf(x) >= 0); const none = !c.some(x => /stdisp_|stpos_|stcolor_/.test(x)); return has && none; })());
chk('D : sous-titres DÉFINITIF — AUCUN on/off, AUCUNE saisie texte', (() => { const v = NAV.view({ screen: 'block', block: { screen: 'video', key: 'soustitres' } }, fvid, {}); const c = cbs(v); return c.indexOf('R0_SET_ston_on') < 0 && c.indexOf('R0_SET_ston_off') < 0 && c.indexOf('R0_ASK_vi_soustitres') < 0; })());
...
// [SÉLECTION QUI PREND] choisir un numéro de grille -> picksrc avec le bon index relatif (page) -> pose la photo comme source du projet
chk('SELECT : R0_GITEM_1 -> picksrc index 1 + retour au flux', (() => { const r = NAV.reduce('R0_GITEM_1', { screen: 'gallery', srcReturn: 'photo_prompt' }, fimg, { galleryFiles: ['/a.jpg', '/b.jpg', '/c.jpg'] }); return r.op && r.op.type === 'picksrc' && r.op.index === 1 && r.st.screen === 'photo_prompt'; })());
