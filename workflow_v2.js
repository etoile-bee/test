/*chrono v2*/(function(){var __t0=Date.now(),__steps=[],__lip=0,__mont=0,__o=console.log.bind(console);console.log=function(){var s=Array.prototype.join.call(arguments,' ');if(/lipsync/i.test(s))__lip++;if(/montage|shotstack|6\/6/i.test(s))__mont++;if(/etape\s*\d\/\d|\u00e9tape\s*\d\/\d|lipsync|montage|lanc/i.test(s)){__steps.push(((Date.now()-__t0)/1000).toFixed(1)+'s  '+s.replace(/\s+/g,' ').slice(0,55));}return __o.apply(null,arguments);};process.on('exit',function(){var el=((Date.now()-__t0)/1000).toFixed(1);__o('\n\u2500\u2500\u2500\u2500\u2500 BILAN v2 \u2500\u2500\u2500\u2500\u2500');__steps.forEach(function(x){__o('   '+x);});__o('\ud83d\udcb3 Lipsync (payant) x'+__lip+'   |   Montage x'+__mont);__o('\u23f1\ufe0f  Duree totale: '+el+'s');});})();
...
async function generateAudio(script,num){
  ...
  const res=await fetch('https://api.elevenlabs.io/v1/text-to-speech/'+EL_VOICE+'/with-timestamps',{
    ...
    body:JSON.stringify({text:script.replace(/\[pause\]/gi,'<break time="0.8s"/>').replace(/\bpause\b/gi,'').replace(/\u2014/g,' ').replace(/\u2013/g,' '),model_id:'eleven_multilingual_v2',voice_settings:{stability:0.45,similarity_boost:0.82,style:0.3,use_speaker_boost:true}})
  });
  let wt=[];
  if(res.ok){
    ...
    const r2=await fetch('https://api.elevenlabs.io/v1/text-to-speech/'+EL_VOICE,{
      ...
      body:JSON.stringify({text:script.replace(/\[pause\]/gi,'<break time="0.8s"/>').replace(/\bpause\b/gi,'').replace(/\u2014/g,' ').replace(/\u2013/g,' '),model_id:'eleven_multilingual_v2',voice_settings:{stability:0.45,similarity_boost:0.82}})
    });
...
async function renderVideo(lipsyncUrl,wordTimings,keywords,duration,num,reactions){
  ...
  wordTimings=wordTimings.filter(w=>!/pause/i.test(w.word));/*pause-fix v1*/
  ...
  const subClips=chunks.map(c=>({
    asset:{type:'html',html:'<p style="font-family:\'Archivo Black\';font-size:48px;color:#FFFFFF;letter-spacing:0px;margin:0;padding:6px 20px;text-align:center;text-transform:uppercase;">'+c.text+'</p>'/*archivo v1*/,width:720,height:175,background:'transparent'},
    start:c.start,length:c.length,position:'bottom',offset:{x:0,y:0.32} // pos+zoom v3 : sous-titres remontes mi-hauteur
  }));
  ...
  const ZOOMS=[1.22,1.40,1.30,1.50,1.35,1.45]; // intensites qui alternent pour faire respirer
  let vc=[],cur=0,ki=0;
  for(const kw of sorted){
    ...
    const ze=Math.min(zs+3.0,duration); // zoom lent 3s
  ...
  try{
    if(reactions&&reactions.length){
      ...
      for(const _rx of reactions.slice(0,3)){/*react2 v1*/
        ...
        _reactClips.push({asset:{type:'audio',src:_url,volume:0.28},start:_st,length:1.2});
  ...
  const edit={timeline:{fonts:[{src:'https://github.com/google/fonts/raw/main/ofl/archivoblack/ArchivoBlack-Regular.ttf'}],background:'#000000',tracks:_tracks},output:{format:'mp4',resolution:'hd',aspectRatio:'9:16',fps:30}};
